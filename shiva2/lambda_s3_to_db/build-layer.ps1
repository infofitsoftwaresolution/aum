...
$layerZip = Join-Path $projectDir "s3-dat-ingestion-deps-layer.zip"
...
docker run --rm --entrypoint /bin/bash `
  ...
  -lc "set -e; rm -rf /var/task/layer_build; rm -f /var/task/s3-dat-ingestion-deps-layer.zip; mkdir -p /var/task/layer_build/python; pip install -r /var/task/requirements.txt -t /var/task/layer_build/python; python - << 'PY'
import os
import shutil
import zipfile
...
zip_path = '/var/task/s3-dat-ingestion-deps-layer.zip'
